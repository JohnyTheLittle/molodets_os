...
class LCD_1inch3(framebuf.FrameBuffer):
    ...
    def show(self):
        ...
        self.cs(1)
    def printchar(self, letter,xpos,ypos,size,c):
        origin = xpos
        charval = ord(letter)
        #print(charval)
    ...
    def delchar(self, xpos,ypos,size,):
        ...
        if size == 3:
            charwidth = 15
            charheight = 27
        c=self.colour(40,40,40)
        lcd.fill_rect(xpos,ypos,charwidth,charheight,0) #xywh

    def printstring(self, string,xpos,ypos,size,c):   
        if size == 1:
            spacing = 8
        if size == 2:
            spacing = 14
        if size == 3:
            spacing = 18
        for i in string:
            self.printchar(i,xpos,ypos,size,c)
            xpos+=spacing

    def centrestring(self, string,ypos,size,c):
        if size == 1:
            spacing = 8
        if size == 2:
            spacing = 14
        ...
        xpos = int((240-used)/2)
        self.printstring(string,xpos,ypos,size,c)
# ========= End of Improved text system ==================

# ========== Start of Graphics routines =================        
# Colour Mixing Routine
    def colour(self, R,G,B): # Compact method!
...
lcd.fill(lcd.colour(0,0,0)) # BLACK 
lcd.show()